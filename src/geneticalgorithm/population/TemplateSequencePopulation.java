 ...
 * @author Pierre REN
 */
public abstract class TemplateSequencePopulation extends Population<SequenceChromosome> {

    public TemplateSequencePopulation(int NP) {
        super(NP);
    }
    ...
    public void getGeneticPopulationWithProportionalSelection() {
        ...
        for (int i = 0; i < index.length; i++) {
            try {
                geneticPopulation.add((SequenceChromosome) population.get(index[i]).clone());
            } catch (CloneNotSupportedException ex) {
                Logger.getLogger(TemplateSequencePopulation.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
        ...
        if (searchForMax) {
            if (bestGenerationFitness > bestFitnessEver) {
                ...
                for (int i = 0; i < bestGenerationIndividual.size(); i++) {
                    try {
                        bestIndividualEver.add((SequenceChromosome) bestGenerationIndividual.get(i).clone());
                    } catch (CloneNotSupportedException ex) {
                        Logger.getLogger(TemplateSequencePopulation.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
    ...
    public void cycleCrossover(double Pc) {
        if (Math.random() < Pc) {
            for (int i = 0; i < geneticPopulation.size(); i += 2) {
                Crossover.cycle(geneticPopulation.get(i), geneticPopulation.get(i + 1));
            }
        }
    }

    public void orderCrossover(double Pc) {
        if (Math.random() < Pc) {
            for (int i = 0; i < geneticPopulation.size(); i += 2) {
                Crossover.order(geneticPopulation.get(i), geneticPopulation.get(i + 1));
            }
        }
    }

    public void partiallyMappedCrossover(double Pc) {
        if (Math.random() < Pc) {
            for (int i = 0; i < geneticPopulation.size(); i += 2) {
                Crossover.partiallyMapped(geneticPopulation.get(i), geneticPopulation.get(i + 1));
            }
        }
    }
    ...
    public void shiftMutation(double Pm) {
        for (int i = 0; i < geneticPopulation.size(); i++) {
            geneticPopulation.get(i).shiftMutation(Pm);
        }
    }

    public void transpositionMutation(double Pm) {
        for (int i = 0; i < geneticPopulation.size(); i++) {
            geneticPopulation.get(i).transpositionMutation(Pm);
        }
    }
    ...
    public void getNextGenerationWithoutSelection() {
        try {
            ...
            population = (ArrayList<SequenceChromosome>) geneticPopulation.clone();
    ...
    public void getNextGenerationWithProportionalSelection() {
        ...
        try {
            for (int i = 0; i < population.size(); i++) {
                population.set(i, (SequenceChromosome) geneticPopulation.get(index[i]).clone());
