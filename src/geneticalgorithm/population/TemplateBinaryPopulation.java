 ...
 * @author Pierre REN
 */
public abstract class TemplateBinaryPopulation extends Population<BinaryChromosome> {

    public TemplateBinaryPopulation(int NP) {
        super(NP);
    }
    ...
    public void getGeneticPopulationWithProportionalSelection() {
        ...
        for (int i = 0; i < index.length; i++) {
            try {
                geneticPopulation.add((BinaryChromosome) population.get(index[i]).clone());
            } catch (CloneNotSupportedException ex) {
                Logger.getLogger(TemplateBinaryPopulation.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
        ...
        if (searchForMax) {
            if (bestGenerationFitness > bestFitnessEver) {
                ...
                for (int i = 0; i < bestGenerationIndividual.size(); i++) {
                    try {
                        bestIndividualEver.add((BinaryChromosome) bestGenerationIndividual.get(i).clone());
                    } catch (CloneNotSupportedException ex) {
                        Logger.getLogger(TemplateBinaryPopulation.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
    ...
    public void getNextGenerationWithoutSelection() {
        try {
            ...
            population = (ArrayList<BinaryChromosome>) geneticPopulation.clone();
    ...
    public void getNextGenerationWithProportionalSelection() {
        ...
        try {
            for (int i = 0; i < population.size(); i++) {
                population.set(i, (BinaryChromosome) geneticPopulation.get(index[i]).clone());
