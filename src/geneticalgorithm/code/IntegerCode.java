 ...
 */
package geneticalgorithm.code;

import tool.Random2Cutting;
    ...
    public void setBound(int[] Nmax) {
        try {
            if (code.length != Nmax.length) {
                throw new Exception("Inequal lengths of code and bound");
            }
    ...
    public void random() {
        try {
            if (upperLimits == null && lowerLimits == null) {
                ...
                throw new Exception("Wrong dimensions of code, lowerlimits and upperlimits.");
            }
        } catch (Exception e) {
            e.printStackTrace(System.out);
        }
